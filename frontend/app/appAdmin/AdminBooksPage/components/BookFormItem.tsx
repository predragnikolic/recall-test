...
export function BookFormItem({
  ...
  return (
    <form onSubmit={save} className={className}>
        ...
        >
            ...
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <Input
                  ...
                  endContent={<p className="whitespace-nowrap self-end  less-nice-font">.00 {/*TODO don't hard code decimals*/}</p>}
                  isInvalid={Boolean(formState.errors.price)}
                  classNames={{
                    inputWrapper: "bg-transparent shadow-none py-1 w-[10ch]",
                    input: "text-right",
                  }}
                />
              )}
          ...
          <div className="flex items-center justify-center gap-2">
            <Controller
              name="status"
              control={control}
              render={({ field: { onChange, value } }) => <BookStatusPopover value={value} onChange={onChange} />}
            />

            <Button variant="light" isIconOnly onPress={() => setShowMore(!showMore)}>
          ...
          {(formState.isDirty || formType === "create") && (
            <div className="flex justify-end gap-3 col-span-full">
              ...
              <Button
                ...
                endContent={formState.isSubmitting && <Spinner color="white" size="sm" />}
                type="submit"
                color="primary"
                className="uppercase"
              >
      ...
      <Card className={classNames("mx-4 rounded-t-none transition-all relative", showMore && "mb-3")}>
        <AutoAnimateHeight ease="ease-in-out">
          <section className={classNames("flex flex-col gap-3", showMore ? "h-[auto] overflow-y-auto" : "h-0")}>
            {showMore && (
